      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: '#4f46e5' }]}>{classes.length}</Text>
            <Text style={styles.statLabel}>Total Classes</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: '#10b981' }]}>{classes.reduce((a, b) => a + (parseInt(b.totalStudents) || 0), 0)}</Text>
            <Text style={styles.statLabel}>Total Students</Text>
          </View>
        </View>

        <TextInput style={styles.searchInput} placeholder="Search classes..." placeholderTextColor="#555b7a" value={search} onChangeText={setSearch} />
            ...
            <View key={cls.id} style={styles.classCard}>
              ...
              <View style={styles.details}>
                <View style={styles.detailRow}>
                  ...
                  <Text style={styles.detailText}>{cls.lecturerName || 'TBA'}</Text>
                </View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailIcon}>V</Text>
                  <Text style={styles.detailText}>{cls.venue || 'N/A'}</Text>
                </View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailIcon}>T</Text>
                  <Text style={styles.detailText}>{cls.scheduledTime || 'N/A'}</Text>
                </View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailIcon}>S</Text>
                  <Text style={styles.detailText}>{cls.totalStudents || 0} Students</Text>
                </View>
              </View>
            </View>
          ))
...
const styles = StyleSheet.create({
  ...
  dayBtn: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 20, borderWidth: 0.5, borderColor: '#2a2f5c', marginRight: 8, backgroundColor: '#1a1f3c' },
  ...
  classCard: { backgroundColor: '#1a1f3c', borderRadius: 16, padding: 16, marginBottom: 14, borderWidth: 0.5, borderColor: '#2a2f5c' },
