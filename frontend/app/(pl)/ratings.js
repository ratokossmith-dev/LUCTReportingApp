...
export default function PLRatings() {
  ...
  const [lecturers, setLecturers] = useState(['All']);
  ...
  const loadRatings = async () => {
    try {
      ...
      setRatings(data);
      const unique = ['All', ...new Set(data.map((r) => r.lecturerName).filter(Boolean))];
      setLecturers(unique);
    } catch (e) {
      console.log('Error:', e);
    }
  ...
  const filtered = ratings.filter((r) => {
    const matchLecturer = selected === 'All' || r.lecturerName === selected;
    const matchSearch =
      !search ||
      (r.studentName || '').toLowerCase().includes(search.toLowerCase()) ||
      (r.lecturerName || '').toLowerCase().includes(search.toLowerCase()) ||
      (r.comment || '').toLowerCase().includes(search.toLowerCase());
    return matchLecturer && matchSearch;
  });
...
const styles = StyleSheet.create({
  ...
  overallCard: { backgroundColor: '#1a1f3c', borderRadius: 16, padding: 20, marginBottom: 16, alignItems: 'center', borderWidth: 0.5, borderColor: '#2a2f5c' },
