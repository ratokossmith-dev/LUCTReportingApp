...
import { getAllReports, deleteReport } from '../../config/firestore';
...
export default function PLReports() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await exportToExcel(exportData, `Reports_${new Date().toISOString().split('T')[0]}`, 'Reports');
    } catch (error) {
  ...
  const handleDeleteReport = (reportId) => {
    console.log('Delete report pressed | id:', reportId);
    Alert.alert(
      'Delete Report',
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            console.log('Confirmed delete for:', reportId);
            try {
              ...
              console.log('Delete report error:', e.message);
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: '#4f46e5' }]}>{reports.length}</Text>
            <Text style={styles.statLabel}>Total</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: '#10b981' }]}>{reports.filter((r) => r.status === 'Reviewed').length}</Text>
            <Text style={styles.statLabel}>Reviewed</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: '#f59e0b' }]}>{reports.filter((r) => r.status !== 'Reviewed').length}</Text>
            <Text style={styles.statLabel}>Pending</Text>
          </View>
        </View>

        <TextInput style={styles.searchInput} placeholder="Search reports..." placeholderTextColor="#555b7a" value={search} onChangeText={setSearch} />
            ...
            <View key={report.id} style={styles.reportCard}>
              <View style={styles.reportHeader}>
                ...
                <View style={styles.reportActions}>
                  <View style={[styles.statusBadge, report.status === 'Reviewed' ? styles.statusReviewed : styles.statusPending]}>
                    <Text style={[styles.statusText, report.status === 'Reviewed' ? styles.statusTextReviewed : styles.statusTextPending]}>
              ...
              <Text style={styles.reportCourse}>{report.courseCode} - {report.courseName}</Text>
              <View style={styles.reportMeta}>
                <Text style={styles.metaText}>{report.lecturerName}</Text>
                <Text style={styles.metaText}>{report.dateOfLecture}</Text>
                <Text style={styles.metaText}>{report.actualStudentsPresent}/{report.totalRegisteredStudents} students</Text>
                <Text style={styles.metaText}>Week {report.weekOfReporting}</Text>
              </View>
              {report.prlFeedback ? (
...
const styles = StyleSheet.create({
  ...
  exportBtnDisabled: { backgroundColor: '#6b7280', opacity: 0.7 },
  ...
  searchInput: { backgroundColor: '#1a1f3c', borderRadius: 12, padding: 12, color: '#fff', marginBottom: 14, borderWidth: 0.5, borderColor: '#2a2f5c', fontSize: 14 },
  ...
  reportCard: { backgroundColor: '#1a1f3c', borderRadius: 16, padding: 16, marginBottom: 14, borderWidth: 0.5, borderColor: '#2a2f5c' },
  ...
  reportTopic: { color: '#fff', fontSize: 16, fontWeight: '600', marginBottom: 4 },
  reportCourse: { color: '#6b7280', fontSize: 12, marginBottom: 10 },
  reportMeta: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 12 },
  metaText: { color: '#9ca3af', fontSize: 12 },
  feedbackBox: { backgroundColor: '#0a0f2c', borderRadius: 10, padding: 10, borderWidth: 0.5, borderColor: '#10b981' },
