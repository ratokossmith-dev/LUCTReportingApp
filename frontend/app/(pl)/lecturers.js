...
import { addLecturer, getAllLecturers, deleteUser } from '../../config/firestore';
...
export default function PLLecturers() {
  ...
  const loadLecturers = async () => {
    try {
      ...
      console.log('Load lecturers error:', e);
    }
    setLoading(false);
  };

  const handleDeleteLecturer = (lecturerId, lecturerName) => {
    Alert.alert(
      'Delete Lecturer',
      `Are you sure you want to delete "${lecturerName}"? This will remove them from all courses and delete their reports and ratings.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              await deleteUser(lecturerId);
              Alert.alert('Success', 'Lecturer deleted successfully');
              loadLecturers();
            } catch (e) {
              console.log('Delete lecturer error:', e);
  ...
  const handleAdd = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        `Name: ${newLecturer.name}\nEmail: ${newLecturer.email}\n\nTemporary Password: ${result.tempPassword}\n\nPlease share these login details with the lecturer.`,
        [
          {
            text: 'OK',
            onPress: () => {
      ...
      console.log('Add lecturer error:', e);
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.statsRow}>
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: '#4f46e5' }]}>{lecturers.length}</Text>
            <Text style={styles.statLabel}>Total Lecturers</Text>
          </View>
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: '#10b981' }]}>{lecturers.filter((l) => l.status !== 'Inactive').length}</Text>
            <Text style={styles.statLabel}>Active</Text>
          </View>
        </View>

        <TextInput style={styles.searchInput} placeholder="Search lecturers..." placeholderTextColor="#555b7a" value={search} onChangeText={setSearch} />

        <Text style={styles.sectionTitle}>All Lecturers ({filtered.length})</Text>
            ...
            <View key={lecturer.id} style={styles.lecturerCard}>
              <View style={styles.lecturerHeader}>
                ...
                <View style={styles.lecturerInfo}>
                  ...
                  <Text style={styles.lecturerFaculty}>{lecturer.facultyName || 'Faculty of ICT'}</Text>
                </View>
                <View style={styles.lecturerActions}>
                  <View style={styles.statusBadge}>
                    <Text style={styles.statusText}>{lecturer.status === 'Inactive' ? 'Inactive' : 'Active'}</Text>
                  </View>
                  <TouchableOpacity style={styles.deleteBtn} onPress={() => handleDeleteLecturer(lecturer.id, lecturer.name)}>
              ...
              {lecturer.tempPassword && (
                <View style={styles.passwordHint}>
                  <Text style={styles.passwordHintText}>Temp password: {lecturer.tempPassword}</Text>
                </View>
              )}
            </View>
          ))
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSubtitle}>A temporary password will be generated automatically</Text>

            <TextInput style={styles.modalInput} placeholder="Full Name *" placeholderTextColor="#555b7a" value={newLecturer.name} onChangeText={(v) => setNewLecturer((p) => ({ ...p, name: v }))} />

            <TextInput style={styles.modalInput} placeholder="Email Address *" placeholderTextColor="#555b7a" value={newLecturer.email} onChangeText={(v) => setNewLecturer((p) => ({ ...p, email: v }))} keyboardType="email-address" autoCapitalize="none" />

            <TextInput style={styles.modalInput} placeholder="Faculty" placeholderTextColor="#555b7a" value={newLecturer.facultyName} onChangeText={(v) => setNewLecturer((p) => ({ ...p, facultyName: v }))} />

            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => { setModalVisible(false); setNewLecturer({ name: '', email: '', facultyName: 'Faculty of ICT' }); }}>
                <Text style={styles.cancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.submitBtn, saving && { backgroundColor: '#3730a3' }]} onPress={handleAdd} disabled={saving}>
                {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitText}>Create and Send</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  addBtn: { backgroundColor: '#4f46e5', borderRadius: 10, paddingVertical: 8, paddingHorizontal: 14 },
  ...
  lecturerActions: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  deleteBtn: { backgroundColor: '#ef4444', borderRadius: 8, paddingVertical: 6, paddingHorizontal: 12 },
  deleteBtnText: { color: '#fff', fontSize: 12, fontWeight: '600' },
  passwordHint: { marginTop: 10, paddingTop: 8, borderTopWidth: 0.5, borderTopColor: '#2a2f5c' },
  passwordHintText: { color: '#f59e0b', fontSize: 11 },
  ...
  modalInput: { backgroundColor: '#0a0f2c', borderRadius: 12, padding: 14, color: '#fff', marginBottom: 12, borderWidth: 0.5, borderColor: '#2a2f5c', fontSize: 14 },
