...
import { getReportsByLecturer, getRatingsByLecturer } from '../../config/firestore';
...
export default function LecturerMonitoring() {
  ...
  const avgAtt = reports.length > 0
    ? Math.round(
        reports.reduce((sum, report) => {
          const rate = (report.actualStudentsPresent / report.totalRegisteredStudents) * 100;
          ...
          <>
            <View style={s.grid}>
              <View style={s.statCard}>
                <Text style={[s.statVal, { color: '#4f46e5' }]}>{reports.length}</Text>
                <Text style={s.statLabel}>Reports Submitted</Text>
              </View>
              <View style={s.statCard}>
                <Text style={[s.statVal, { color: '#10b981' }]}>{reviewed}</Text>
              ...
              <View style={s.statCard}>
                <Text style={[s.statVal, { color: '#ec4899' }]}>{avgRating}</Text>
                <Text style={s.statLabel}>Avg Rating</Text>
              </View>
            </View>

            <Text style={s.section}>
              My Reports and PRL Feedback ({reports.length})
            </Text>
            ...
            {reports.length === 0 ? (
              ...
              reports.map((report, index) => (
                <View key={report.id || index} style={s.reportCard}>
                  <View style={s.reportHeader}>
                    <View style={s.classBadge}>
                      <Text style={s.classBadgeText}>{report.className || 'N/A'}</Text>
                    </View>
                    <View
                      style={[
                        s.statusBadge,
                  ...
                  <Text style={s.topic}>{report.topicTaught || 'No topic'}</Text>
                  <View
                    style={{ flexDirection: 'row', gap: 16, marginBottom: 8, flexWrap: 'wrap' }}
                  >
                    <Text style={s.meta}>Week {report.weekOfReporting || 'N/A'}</Text>
                    <Text style={s.meta}>Date: {report.dateOfLecture || 'N/A'}</Text>
                    <Text style={s.meta}>
                      Students: {report.actualStudentsPresent || 0}/{report.totalRegisteredStudents || 0}
                    </Text>
                  </View>
                  {report.prlFeedback ? (
                    <View style={s.feedbackBox}>
                      <Text style={s.feedbackLabel}>PRL Feedback:</Text>
                      <Text style={s.feedbackText}>{report.prlFeedback}</Text>
                    </View>
                  ) : (
