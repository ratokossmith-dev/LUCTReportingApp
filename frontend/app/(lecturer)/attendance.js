...
export default function LecturerAttendance() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const c = await getClassesByLecturer();
        setClasses(c);
      } catch (e) {
      ...
      <ScrollView style={s.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.statsRow}>
          <View style={s.statBox}>
            <Text style={[s.statVal, { color: '#10b981' }]}>{presentCount}</Text>
            <Text style={s.statLabel}>Present</Text>
          </View>
          <View style={s.statBox}>
            <Text style={[s.statVal, { color: '#ef4444' }]}>{students.length - presentCount}</Text>
            <Text style={s.statLabel}>Absent</Text>
          </View>
          <View style={s.statBox}>
            <Text style={[s.statVal, { color: '#4f46e5' }]}>{students.length}</Text>
        ...
        {loading ? (
          ...
          <View style={s.empty}>
            <Text style={s.emptyText}>No classes found. Add a class first.</Text>
          </View>
        ) : (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
            {classes.map((cls) => (
              <TouchableOpacity
                key={cls.id}
                style={[s.classBtn, selectedClass?.id === cls.id && s.classBtnActive]}
                onPress={() => handleClassSelect(cls)}
              >
                <Text style={[s.classText, selectedClass?.id === cls.id && s.classTextActive]}>
                  {cls.className}
                </Text>
                <Text style={[s.classSub, selectedClass?.id === cls.id && { color: 'rgba(255,255,255,0.7)' }]}>
                  {cls.day}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <>
            ...
            {loadingStudents ? (
              ...
              <View style={s.empty}>
                ...
                <Text style={s.emptyText}>Students will be automatically enrolled when they register.</Text>
              </View>
            ) : (
              <>
                <TextInput
                ...
                <View style={s.markAllRow}>
                  <TouchableOpacity style={s.markAllBtn} onPress={() => markAll(true)}>
                    <Text style={s.markAllText}>Mark All Present</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[s.markAllBtn, s.markAllAbsent]} onPress={() => markAll(false)}>
                    <Text style={[s.markAllText, { color: '#ef4444' }]}>Mark All Absent</Text>
                  </TouchableOpacity>
                </View>
                ...
                {filteredStudents.map((st) => (
                  <TouchableOpacity
                    key={st.id}
                    style={[s.studentCard, attendance[st.id] === true && s.studentCardPresent]}
                    onPress={() => toggleAttendance(st.id)}
                    activeOpacity={0.7}
                  >
                    <View style={[s.avatar, attendance[st.id] === true && s.avatarPresent]}>
                      <Text style={s.avatarText}>{st.name?.charAt(0)?.toUpperCase() || 'S'}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={s.studentName}>{st.name}</Text>
                      <Text style={s.studentEmail}>{st.email}</Text>
                    </View>
                    <View style={[s.badge, attendance[st.id] === true ? s.badgePresent : s.badgeAbsent]}>
                      <Text style={[s.badgeText, attendance[st.id] === true ? s.textPresent : s.textAbsent]}>
                ...
                <TouchableOpacity style={[s.saveBtn, saving && s.saveBtnDisabled]} onPress={handleSave} disabled={saving}>
                  {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.saveBtnText}>Save Attendance ({presentCount}/{students.length})</Text>}
                </TouchableOpacity>
              </>
            )}
          </>
...
const s = StyleSheet.create({
  ...
  empty: { backgroundColor: '#1a1f3c', borderRadius: 14, padding: 28, alignItems: 'center', borderWidth: 0.5, borderColor: '#2a2f5c', marginBottom: 16 },
  emptyIcon: { fontSize: 32, marginBottom: 8, color: '#6b7280' },
  ...
  classBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 14, borderWidth: 0.5, borderColor: '#2a2f5c', marginRight: 10, backgroundColor: '#1a1f3c', minWidth: 80, alignItems: 'center' },
  ...
  dateBox: { backgroundColor: '#1a1f3c', borderRadius: 12, padding: 14, marginBottom: 20, borderWidth: 0.5, borderColor: '#2a2f5c' },
  dateLabel: { color: '#fff', fontSize: 14, fontWeight: '600' },
  classInfo: { color: '#6b7280', fontSize: 12, marginTop: 4 },
  loadingBox: { backgroundColor: '#1a1f3c', borderRadius: 14, padding: 24, alignItems: 'center', borderWidth: 0.5, borderColor: '#2a2f5c', marginBottom: 16, gap: 12 },
  ...
  markAllBtn: { flex: 1, backgroundColor: '#1a1f3c', borderRadius: 10, padding: 10, alignItems: 'center', borderWidth: 0.5, borderColor: '#10b981' },
  ...
  studentCard: { backgroundColor: '#1a1f3c', borderRadius: 14, padding: 14, marginBottom: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 0.5, borderColor: '#2a2f5c' },
  studentCardPresent: { borderColor: '#10b981', backgroundColor: '#1a2a1f' },
  avatar: { width: 40, height: 40, backgroundColor: '#2a2f5c', borderRadius: 20, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ...
  badge: { paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, borderWidth: 0.5, alignItems: 'center' },
  badgePresent: { borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.1)' },
  badgeAbsent: { borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.05)' },
  badgeText: { fontSize: 12, fontWeight: '600' },
  textPresent: { color: '#10b981' },
  textAbsent: { color: '#ef4444' },
  saveBtn: { backgroundColor: '#4f46e5', borderRadius: 14, padding: 16, alignItems: 'center', marginBottom: 32, marginTop: 8 },
