...
export default function LecturerRatings() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getRatingsByLecturer();
        setRatings(data);
      } catch (e) {
  ...
  const handleExport = async () => {
    ...
    await exportToExcel(exportData, `My_Ratings_${new Date().toISOString().split('T')[0]}`, 'Ratings');
  };
      ...
      <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.overallCard}>
          ...
          <View style={{ flexDirection: 'row', gap: 4, marginBottom: 8 }}>
            {[1, 2, 3, 4, 5].map((st) => (
              <Text key={st} style={{ fontSize: 24, color: st <= Math.round(parseFloat(avg)) ? '#f59e0b' : '#2a2f5c' }}>★</Text>
            ))}
          </View>
          <Text style={s.overallLabel}>Based on {ratings.length} student reviews</Text>
            ...
            {[5, 4, 3, 2, 1].map((star) => {
              ...
              return (
                <View key={star} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <Text style={{ color: '#f59e0b', fontSize: 12, width: 30 }}>{star} ★</Text>
                  <View style={{ flex: 1, height: 6, backgroundColor: '#0a0f2c', borderRadius: 3 }}>
                    <View style={{ width: `${pct}%`, height: 6, backgroundColor: '#f59e0b', borderRadius: 3 }} />
                  </View>
                  <Text style={{ color: '#6b7280', fontSize: 12, width: 20, textAlign: 'right' }}>{count}</Text>
                </View>
              );
            })}
          </View>
          ...
          ratings.map((item, i) => (
            <View key={item.id || i} style={s.reviewCard}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8, gap: 10 }}>
                <View style={s.avatar}>
                  <Text style={s.avatarText}>{item.studentName?.charAt(0) || 'S'}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={s.reviewName}>{item.studentName || 'Student'}</Text>
                  <Text style={s.reviewDate}>{item.course || 'N/A'}</Text>
                </View>
                <View style={{ flexDirection: 'row', gap: 2 }}>
                  {[1, 2, 3, 4, 5].map((st) => (
                    <Text key={st} style={{ fontSize: 14, color: st <= item.rating ? '#f59e0b' : '#2a2f5c' }}>★</Text>
                  ))}
                </View>
              </View>
...
const s = StyleSheet.create({
  ...
  reviewCard: { backgroundColor: '#1a1f3c', borderRadius: 14, padding: 14, marginBottom: 12, borderWidth: 0.5, borderColor: '#2a2f5c' },
