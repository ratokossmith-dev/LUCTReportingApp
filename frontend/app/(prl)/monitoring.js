    ...
    (async () => {
      try {
        ...
        lects.forEach((l) => {
          ...
          const avg = rat.length > 0
            ? (rat.reduce((a, b) => a + b.rating, 0) / rat.length).toFixed(1)
            : 'N/A';

          const att = r.length > 0
            ? Math.round(
                r.reduce(
                  (a, b) => a + (b.actualStudentsPresent / b.totalRegisteredStudents) * 100,
                  0
                ) / r.length
              )
            : 0;
                ...
                <View style={s.cardHeader}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={s.lecStat}>
                      Reports: {st.reports || 0} - Reviewed: {st.reviewed || 0}
                    </Text>
                  </View>
                  <View style={s.ratingBadge}>
                    <Text style={s.ratingText}>Rating: {st.rating || 'N/A'}</Text>
                  </View>
                </View>
                <View style={s.progRow}>
                  <Text style={s.progLabel}>Avg Attendance</Text>
                  <Text style={[s.progVal, { color: color(st.attendance || 0) }]}>
                    {st.attendance || 0}%
                  </Text>
                </View>
