    ...
    <View style={s.safe}>
      <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
        ...
        {Object.keys(byClass).length > 0 && (
          <>
            ...
            {Object.entries(byClass).map(([className, data]) => {
              const classRate = data.total > 0 ? Math.round((data.present / data.total) * 100) : 0;
              const classColor = classRate >= 90 ? '#10b981' : classRate >= 75 ? '#f59e0b' : '#ef4444';
              return (
                <View key={className} style={s.classCard}>
                  ...
                  <View style={s.progressBar}>
                    <View
                      style={[s.progressFill, { width: `${classRate}%`, backgroundColor: classColor }]}
                    />
                  </View>
        ...
        {loading ? (
          ...
          filtered.map((r, i) => (
            <View key={r.id || i} style={[s.recordCard, r.present ? s.recordPresent : s.recordAbsent]}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.recordLecturer}>
                  Lecturer: {r.lecturerName || 'Lecturer'}
                </Text>
                <Text style={s.recordDate}>Date: {r.date || 'N/A'}</Text>
              </View>
              <View style={[s.badge, r.present ? s.badgePresent : s.badgeAbsent]}>
                <Text style={[s.badgeText, r.present ? s.textPresent : s.textAbsent]}>
...
const s = StyleSheet.create({
  ...
  overallLabel: { color: '#6b7280', fontSize: 13, marginTop: 4, marginBottom: 16 },
  ...
  recordClass: { color: '#fff', fontSize: 14, fontWeight: '600', marginBottom: 2 },
  ...
  badge: { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10, borderWidth: 0.5, alignItems: 'center', minWidth: 80 },
