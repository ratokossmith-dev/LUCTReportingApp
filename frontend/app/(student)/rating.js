...
export default function StudentRating() {
  ...
  const loadData = async () => {
    try {
      const myClasses = await getClassesByStudent();
      const lecturerIds = [...new Set(myClasses.map((c) => c.lecturerId).filter(Boolean))];
      const allLecturers = await getAllLecturers();
      const myLecturers = lecturerIds.length > 0
        ? allLecturers.filter((l) => lecturerIds.includes(l.id))
        : [];
      const myR = await getStudentRatings();
        ...
        {loading ? (
          ...
          lecturers.map((l) => {
            ...
            return (
              <TouchableOpacity
                key={l.id}
                style={[s.lecCard, selected?.id === l.id && s.lecCardActive, rated && s.lecCardRated]}
                onPress={() => !rated && setSelected(l)}
              >
                <View style={s.lecAvatar}>
                  <Text style={s.lecAvatarText}>{l.name?.charAt(0) || 'L'}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={s.lecName}>{l.name}</Text>
                  <Text style={s.lecEmail}>{l.email}</Text>
                  {rated && (
                    <View style={{ flexDirection: 'row', gap: 2, marginTop: 4, alignItems: 'center' }}>
                      {[1, 2, 3, 4, 5].map((st) => (
                        <Text
                          key={st}
                          style={{ fontSize: 12, color: st <= (myRating?.rating || 0) ? '#f59e0b' : '#2a2f5c' }}
                        >
                          ★
                        </Text>
                      ))}
                ...
                {selected?.id === l.id && !rated && <Text style={s.check}>✓</Text>}
              </TouchableOpacity>
            );
          })
        )}
        ...
        {selected && !alreadyRated(selected.id) && (
          <>
            ...
            <View style={s.starsCard}>
              <Text style={s.starsLabel}>{rating === 0 ? 'Tap a star to rate' : ratingLabels[rating]}</Text>
              <View style={{ flexDirection: 'row', gap: 8 }}>
                {[1, 2, 3, 4, 5].map((st) => (
                  <TouchableOpacity key={st} onPress={() => setRating(st)}>
                    <Text style={{ fontSize: 40, color: st <= rating ? '#f59e0b' : '#2a2f5c' }}>★</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            <TouchableOpacity style={[s.submitBtn, saving && s.submitDisabled]} onPress={handleSubmit} disabled={saving}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.submitText}>Submit Rating</Text>}
            </TouchableOpacity>
          </>
        )}
              ...
              <View key={r.id || i} style={s.ratingCard}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 2 }}>
                    {[1, 2, 3, 4, 5].map((st) => (
                      <Text key={st} style={{ fontSize: 14, color: st <= r.rating ? '#f59e0b' : '#2a2f5c' }}>★</Text>
                    ))}
                  </View>
                </View>
...
const s = StyleSheet.create({
  ...
  lecAvatar: { width: 44, height: 44, backgroundColor: '#4f46e5', borderRadius: 22, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ...
  starsCard: { backgroundColor: '#1a1f3c', borderRadius: 14, padding: 20, marginBottom: 20, alignItems: 'center', borderWidth: 0.5, borderColor: '#2a2f5c' },
  ...
  submitBtn: { backgroundColor: '#4f46e5', borderRadius: 14, padding: 16, alignItems: 'center', marginBottom: 24 },
  submitDisabled: { backgroundColor: '#3730a3' },
  submitText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  ratingCard: { backgroundColor: '#1a1f3c', borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 0.5, borderColor: '#2a2f5c' },
