...
export default function StudentMonitoring() {
  ...
  const classStats = classes.map((cls) => {
    ...
    const classRate = classAtt.length > 0 ? Math.round((classPresent / classAtt.length) * 100) : 0;
      ...
      <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <>
            ...
            <Text style={s.section}>My Classes and Attendance</Text>
            {filteredClassStats.length === 0 ? null : filteredClassStats.map((cls) => {
              const color = cls.classRate >= 90 ? '#10b981' : cls.classRate >= 75 ? '#f59e0b' : '#ef4444';
              return (
                <View key={cls.id} style={s.classCard}>
                  ...
                  <Text style={s.lecturerName}>
                    Lecturer: {cls.lecturerName || 'TBA'}
                  </Text>
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                    {cls.day ? <Text style={s.detail}>Day: {cls.day}</Text> : null}
                    {cls.scheduledTime ? <Text style={s.detail}>Time: {cls.scheduledTime}</Text> : null}
                    {cls.venue ? <Text style={s.detail}>Venue: {cls.venue}</Text> : null}
                  </View>
                  <View style={s.progressBar}>
                    <View style={[s.progressFill, { width: `${cls.classRate}%`, backgroundColor: color }]} />
                  </View>
            ...
            {filteredCourses.length === 0 ? null : filteredCourses.map((c, i) => (
              <View key={c.id || i} style={s.courseCard}>
                <View style={s.courseCodeBadge}>
                  <Text style={s.courseCodeText}>{c.courseCode || 'N/A'}</Text>
                </View>
                <Text style={s.courseCardName}>{c.courseName || 'N/A'}</Text>
                <Text style={s.courseCardSub}>
                  Lecturer: {c.lecturerName || 'TBA'} - Semester {c.semester || 'N/A'}
                </Text>
              </View>
            ))}
          </>
...
const s = StyleSheet.create({
  ...
  courseName: { color: '#fff', fontSize: 14, fontWeight: '600', marginBottom: 4 },
  lecturerName: { color: '#6b7280', fontSize: 12, marginBottom: 8 },
  detail: { color: '#9ca3af', fontSize: 11, backgroundColor: '#0a0f2c', paddingVertical: 3, paddingHorizontal: 8, borderRadius: 6 },
  progressBar: { height: 6, backgroundColor: '#0a0f2c', borderRadius: 3, marginBottom: 6 },
  progressFill: { height: 6, borderRadius: 3 },
  attStat: { color: '#6b7280', fontSize: 11 },
  courseCard: { backgroundColor: '#1a1f3c', borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 0.5, borderColor: '#2a2f5c' },
  courseCodeBadge: { backgroundColor: '#4f46e5', borderRadius: 8, paddingVertical: 4, paddingHorizontal: 10, alignSelf: 'flex-start', marginBottom: 8 },
  courseCodeText: { color: '#fff', fontSize: 12, fontWeight: '600' },
  courseCardName: { color: '#fff', fontSize: 14, fontWeight: '600', marginBottom: 4 },
